...
def synchronized_quat_bone_transforms_from_fcurves(fcurves_dict, bones):
    bone_fcurves = synchronized_bone_transforms_from_fcurves(fcurves_dict)
    for bn, data in bone_fcurves.items():
        # Bones in Blender always use quaternions for rotations in the pose system
        # If we have euler rotations, we need to convert them, but typically bones use quaternions
        # Check if we have euler rotations and need to convert
        if 'rotation_euler' in data:
            # For bones, we'll assume XYZ euler mode if we need to convert
            # But typically bones should already be in quaternion format
            # If rotation_quaternion is not present, convert from euler
            if 'rotation_quaternion' not in data:
                # Default to XYZ euler mode for conversion
                data["rotation_quaternion"] = {k: euler_to_quat(e, "XYZ").normalized() for k, e in data["rotation_euler"].items()}
